...
# Etiquetas de texto de cada página (se traducen al cambiar de idioma)
...
func _ready() -> void:
	...
	# Conecta los botones de flecha de cada página para navegar entre ellas
...
# Actualiza todos los textos traducibles del tutorial según el idioma activo.
...
# Detiene la música y vuelve a la escena del menú principal.
func _on_go_back_to_menu_pressed() -> void:
	AudioManager.stop_music()
	get_tree().change_scene_to_file("res://scenes/ui/main_menu.tscn")

# Recorre todas las páginas y conecta sus flechas izquierda y derecha al callback correcto.
...
# Conecta la señal de un nodo flecha (Button o Area2D) al callback indicado.
...
# Navega a la página anterior al pulsar la flecha izquierda.
func _on_left_pressed(page_num: int, _a = null, _b = null, _c = null) -> void:
	_show_page(page_num - 1)

# Navega a la página siguiente al pulsar la flecha derecha.
func _on_right_pressed(page_num: int, _a = null, _b = null, _c = null) -> void:
	_show_page(page_num + 1)

# Muestra solo la página indicada y oculta el resto.
# Clampa el número para no salirse del rango de páginas disponibles.
